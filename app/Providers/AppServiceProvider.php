...
//        Paginator::useBootstrap();
        view()->composer('pages.sidebar', function($view){
            ...
            $view->with('lastPosts', Post::orderBy('created_at', 'desc')->active()->take(3)->get());
            $view->with('categories', Category::all());
            $view->with('tags', Tag::all());
            $view->with('admin', User::getFirstPost());
        });
        view()->composer('pages.navbar', function($view){
            $view->with('menus', Category::all());
        });
        view()->composer('pages.footer', function($view){
            $view->with('lastPosts', Post::orderBy('id', 'desc')->active()->take(3)->get());
            $view->with('admin', User::getFirstPost());
        });
